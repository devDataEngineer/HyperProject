...
def test_function_returns_correctly_formatted_table(mock_conn):
    ...
    assert result[0] == [{'a': 'a', 'b': 'b', 'c': 'c', 'd': 'd'}, {'a': 1, 'b': 2, 'c': 3, 'd': 4}]
...
def test_function_correctly_raises_DatabaseError(mock_conn):
        mock_conn.side_effect = Exception(DatabaseError)
...
def test_lambra_handler_returns_list(s3_client):
    with patch ("src.extractlambda.extract.load_all_tables") as p:
        ...
        assert isinstance(lambda_handler("", ""), list)

@pytest.mark.skip()
def test_load_all_tables_returns_list(s3_client):
    pass
